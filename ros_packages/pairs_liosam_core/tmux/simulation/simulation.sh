...
tmux send-keys -t "$P" "$SETUP; "'waitForGazebo; rosservice call /pairs_drone_spawner/spawn "1 $UAV_TYPE --enable-rangefinder --enable-ground-truth --enable-ouster model:=OS0-32 horizontal_samples:=1024 use_gpu:=True --pos 0 0 0 0"' Enter
...
tmux send-keys -t "$P" "$SETUP; "'waitForControl; roslaunch pairs_liosam_core rviz.launch' Enter
